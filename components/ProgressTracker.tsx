    ...
    },
    {
    ...
    },
     {
        ...
        getCurrentProgress: (context) => {
           ...
           const inNaturaCount = context.pantry.filter(i => i.classification === 'in natura').length;
...
const achievements: Achievement[] = [
    ...
    },
    {
        ...
        isUnlocked: (context) => {
            ...
            const inNatura = context.pantry.filter(i => i.classification === 'in natura').length;
            const ultra = context.pantry.filter(i => i.classification === 'ultraprocessado').length;
...
const ProgressTracker: React.FC = () => {
    ...
    return (
        <div>
            ...
            <div className="mb-10">
                <h3 className="text-2xl font-semibold text-brand-text mb-4 flex items-center"><Target className="mr-3 text-brand-primary"/> Missões Atuais</h3>
                        ...
                        return (
                            <div key={mission.id} className="bg-brand-surface p-4 rounded-2xl shadow-edu">
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                    <div className={`h-full rounded-full transition-all duration-500 ${isCompleted ? 'bg-brand-accent-lime' : 'bg-brand-primary'}`} style={{ width: `${percentage}%` }}>
                                    </div>
                                </div>
                            </div>
                        );
            ...
            <div>
                <h3 className="text-2xl font-semibold text-brand-text mb-4 flex items-center"><Medal className="mr-3 text-yellow-500"/> Conquistas</h3>
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
                    {achievements.map(ach => {
                        const unlocked = ach.isUnlocked(context);
                        return (
                            <div key={ach.id} className={`p-4 rounded-2xl text-center transition-all duration-300 flex flex-col items-center justify-center aspect-square ${unlocked ? 'bg-yellow-100/50 shadow-edu' : 'bg-gray-100'}`}>
                                <div className={`p-3 rounded-full mb-2 ${unlocked ? 'bg-yellow-400' : 'bg-gray-300'}`}>
                                    <ach.icon className={`h-8 w-8 transition-colors ${unlocked ? 'text-white' : 'text-gray-500'}`} />
                                </div>
                                <p className={`font-bold text-sm ${unlocked ? 'text-yellow-900' : 'text-brand-text-secondary'}`}>{ach.title}</p>
                            </div>
                        );
                    })}
                </div>
