...
import { Check, Edit, Save, FileCheck2, Package } from 'lucide-react';
    ...
    return (
        <div className="bg-brand-surface rounded-2xl shadow-edu p-4 mt-2 w-full max-w-xs md:max-w-md lg:max-w-lg border border-brand-border">
            <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                {items.map(item => (
                    <div key={item.id} className={`flex items-center gap-2 p-2 rounded-lg transition-colors ${item.isIncluded ? 'bg-green-50' : 'bg-gray-100 opacity-60'}`}>
                        <input
                            type="checkbox"
                        ...
                        {editingId === item.id ? (
                             <>
                                <input type="text" value={item.name} onChange={e => handleInputChange(item.id, 'name', e.target.value)} className="w-full p-1 border rounded text-sm"/>
                                <input type="number" value={item.quantity} onChange={e => handleInputChange(item.id, 'quantity', parseFloat(e.target.value) || 0)} className="w-16 p-1 border rounded text-sm"/>
                                <select 
                                    value={item.unit} 
